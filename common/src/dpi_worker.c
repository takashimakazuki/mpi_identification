...
#define INVALID_FID 0
#define MAX_DPI_DEPTH (1 << 14)
...
static void
set_netflow_record(struct flow_info *flow, const struct worker_ctx *ctx, bool initator)
{
	struct doca_netflow_default_record *record_to_send;

	if (ctx->attr.send_netflow_record == NULL)
		return;
	record_to_send = &flow->record[initator];
		    ...
		    const struct rte_sft_mbuf_info *mbuf_info)
{
	...
	*payload_offset += sizeof(struct rte_ether_hdr);
	...
	if (!mbuf_info->is_ipv6) {
		*payload_offset += (uint32_t)rte_ipv4_hdr_len(mbuf_info->ip4);
		parsing_info->dst_ip.ipv4.s_addr = mbuf_info->ip4->dst_addr;
	} else {
		/* Currently, no IPV6 extension support */
		if (ipv6_ext_hdr_len(mbuf_info->ip6, &l4_protocol) != 0)
			return -1;
		*payload_offset += IPV6_HDR_LEN_NO_EXT;
				...
				const struct rte_sft_5tuple *five_tuple)
{
	...
	if (rte_sft_flow_set_client_obj(ctx->queue_id, key, client_id, flow, &error) != 0)
...
debug_dpi_dequeue_status(int dpi_result, int status_flags, int fid)
{
	if (status_flags & DOCA_DPI_STATUS_LAST_PACKET)
		DOCA_LOG_DBG("Indicates there are no more packets in queue");
...
static void
forward_packets(struct rte_mbuf *packets[], const uint8_t burst_size, const struct worker_ctx *ctx)
{
	uint16_t nb_tx;
	...
	do {
		...
		DOCA_LOG_DBG("Forwarded %d packets to port=0x%x", burst_size, ctx->ingress_port^1);
		packets += nb_tx;
		remaining_packets -= nb_tx;
	} while (packets != NULL && remaining_packets > 0);
}

static void
handle_and_forward_ooo(struct rte_sft_flow_status *sft_status, struct worker_ctx *ctx)
{
	int ret, drained_packets = 0;
	...
	struct flow_info *flow = (struct flow_info *)rte_sft_flow_get_client_obj(ctx->queue_id,
		sft_status->fid, client_id, &error);
...
{
	int packet_number = 0;
	uint8_t non_filtered_packets = 0;
	...
	if (nb_rx != 0) {
		...
		for (packet_number = 0; packet_number < nb_rx; packet_number++) {
			memset(&sft_status, 0, sizeof(sft_status));
...
dpi_worker_lcores_run(int nb_queues, int app_client_id, struct dpi_worker_attr attr)
{
	...
	while ((current_lcore != RTE_MAX_LCORE) && (lcore_index < nb_queues)) {
		...
		ctx->attr = attr;
		if (rte_eal_remote_launch((void *)dpi_worker, (void *)ctx,
						current_lcore) != 0)
