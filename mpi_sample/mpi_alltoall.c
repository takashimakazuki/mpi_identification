...
int main(int argc, char* argv[])
{
    ...
    printf("Process %d, my values = %d, %d, %d, %d.\n", my_rank, my_values[0], my_values[1], my_values[2], my_values[3]);
 
    int buffer_recv[PROCESSES];
    MPI_Alltoall(&my_values, 1, MPI_INT, buffer_recv, 1, MPI_INT, MPI_COMM_WORLD);
    printf("Values collected on process %d: %d, %d, %d, %d.\n", my_rank, buffer_recv[0], buffer_recv[1], buffer_recv[2], buffer_recv[3]);
